...
async function seed() {
  ...
  try {
    // 1. Get owner_id from an existing song
    const sFetch = await fetch(URL_SONGS + "?select=owner_id&limit=1", { headers });
    const songsList = await sFetch.json();
    if (!songsList?.[0]) throw new Error("No songs found to extract owner_id");
    const ownerId = songsList[0].owner_id;
