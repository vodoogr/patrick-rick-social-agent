...
import { Search, Music2, Play, Filter, ChevronDown, Check, Disc3, Loader2 } from "lucide-react";
...
export default function CatalogPage() {
  ...
  }, [query, eraFilter, albumFilter, page]);

  useEffect(() => {
    fetchSongs(true);
  }, [query, eraFilter, albumFilter]);
    ...
    <div className="max-w-7xl mx-auto space-y-8 animate-in fade-in duration-700">
      ...
      <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-4">
        ...
        <div className="relative">
          <button
            onClick={() => { setShowEraDropdown(!showEraDropdown); setShowAlbumDropdown(false); }}
        ...
        <div className="relative">
          <button
            onClick={() => { setShowAlbumDropdown(!showAlbumDropdown); setShowEraDropdown(false); }}
            className="flex items-center gap-2 px-4 py-3 rounded-2xl bg-white/5 border border-white/10 text-sm hover:bg-white/10 transition-colors whitespace-nowrap"
          >
      ...
      <div className="flex items-center justify-between">
        <p className="text-xs text-white/30 font-bold uppercase tracking-widest">
          {total} {total === 1 ? "track" : "tracks"} found
        </p>
      </div>
